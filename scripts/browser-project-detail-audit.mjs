...
const harnessDirectory = join(repositoryRoot, 'src/pages/project-detail-audit');
const harnessPath = join(harnessDirectory, '[slug].astro');
const narrativePath = join(harnessDirectory, '_AuditNarrative.astro');
const assetDirectory = join(repositoryRoot, 'public/project-detail-audit');
const browserDataDirectory = await mkdtemp('/tmp/azwerks-project-detail-browser-audit-');
const processes = [];
const results = [];
const metrics = { buildMilliseconds: {} };

const build = (count) => {
  const started = performance.now();
  const result = spawnSync(process.execPath, [astroCli, 'build'], {
    cwd: repositoryRoot,
    env: { ...process.env, ...(count === undefined ? {} : { PROJECT_DETAIL_AUDIT_COUNT: String(count) }) },
    stdio: 'inherit',
  });
...
const openPage = async ({
  ...
  await session.send('Page.addScriptToEvaluateOnNewDocument', {
    source: `window.__detailCls=0;try{new PerformanceObserver((list)=>{for(const entry of list.getEntries()){if(!entry.hadRecentInput)window.__detailCls+=entry.value}}).observe({type:'layout-shift',buffered:true})}catch{}`,
  });
...
const auditAssets = async () => {
  ...
  let cssBytes = 0;
  let cssGzipBytes = 0;
  let projectJavaScriptBytes = 0;
  for (const file of await readdir(directory)) {
    const source = await readFile(join(directory, file));
    const text = source.toString('utf8');
    if (file.endsWith('.css') && text.includes('.project-detail')) {
      cssBytes += source.byteLength;
      cssGzipBytes += gzipSync(source).byteLength;
    }
    if (file.endsWith('.js') && text.includes('project-detail')) projectJavaScriptBytes += source.byteLength;
  }
  Object.assign(metrics, { cssBytes, cssGzipBytes, projectJavaScriptBytes });
};
...
import { createProjectDetailAuditBundles } from '../../../tests/support/project-detail-fixtures.ts';
import AuditNarrative from './_AuditNarrative.astro';

export const getStaticPaths = () => {
  const requestedCount = Math.max(0, Math.min(10, Number(process.env.PROJECT_DETAIL_AUDIT_COUNT ?? '10')));
  const bundles = createProjectDetailAuditBundles(requestedCount);
  const routes = getProjectDetailRouteRecords(bundles);
  const auditHref = (slug) => \`/project-detail-audit/\${slug}/\`;
...
<ProjectDetailLayout project={project}>
  ...
  <ProjectDetail project={project} Content={AuditNarrative} hasNarrative />
</ProjectDetailLayout>
`;
...
try {
  ...
  await auditAssets();
  metrics.sparseHtmlBytes = (await stat(join(repositoryRoot, 'dist/project-detail-audit/synthetic-detail-05/index.html'))).size;
  metrics.normalHtmlBytes = (await stat(join(repositoryRoot, 'dist/project-detail-audit/synthetic-detail-01/index.html'))).size;
  metrics.longHtmlBytes = (await stat(join(repositoryRoot, 'dist/project-detail-audit/synthetic-detail-06/index.html'))).size;

  start(process.execPath, [astroCli, 'preview', '--host', '127.0.0.1', '--port', '4324']);
  await waitFor(`${siteUrl}/project-detail-audit/synthetic-detail-01/`);
  ...
  for (let index = 0; index < categories.length; index += 1) {
    ...
    const page = await openPage({ path: `/project-detail-audit/synthetic-detail-${serial}/` });
  ...
  const rich = await openPage({ path: '/project-detail-audit/synthetic-detail-01/' });
  ...
  const long = await openPage({ path: '/project-detail-audit/synthetic-detail-06/', width: 320, textScale: true });
  ...
  const mediaPage = await openPage({ path: '/project-detail-audit/synthetic-detail-04/' });
  ...
  record('video transcript contract', { 'native video controls': mediaData.video, 'public poster': mediaData.poster === '/project-detail-audit/synthetic-media.svg', 'visible transcript': mediaData.transcript });
  await closePage(mediaPage);

  const artPage = await openPage({ path: '/project-detail-audit/synthetic-detail-03/' });
  ...
  const sparse = await openPage({ path: '/project-detail-audit/synthetic-detail-05/', javaScriptDisabled: true, width: 375 });
  ...
  for (const [name, options] of [
    ...
    const page = await openPage({ path: '/project-detail-audit/synthetic-detail-02/', ...options });
  ...
  for (const width of [320, 375, 768, 1024, 1440, 1600]) {
    const page = await openPage({ path: '/project-detail-audit/synthetic-detail-06/', width, height: width === 768 ? 420 : 900 });
  ...
  const zoomed = await openPage({ path: '/project-detail-audit/synthetic-detail-06/', width: 375 });
  ...
  const missing = await fetch(`${siteUrl}/project-detail-audit/unknown-slug/`);
  record('unknown slug', { 'missing route is 404': missing.status === 404 });

  record('performance budgets', {
    'project CSS at most 18KB gzip': metrics.cssGzipBytes <= 18 * 1024,
    'project-specific JavaScript is zero': metrics.projectJavaScriptBytes === 0,
...
console.log(`Project detail browser audit: ${results.length - failed.length}/${results.length} scenarios passed.`);
console.log(`Project detail metrics: ${JSON.stringify(metrics)}`);
